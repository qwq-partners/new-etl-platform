...
| 등급 | 건수 | 무엇 | 상태 |
|---|---|---|---|
| **P0** | **2** | D-01 조회 경로 선택 · **A-13 시각 필드의 저장 시간대** | **둘 다 v1.2.4 에서 처리** |
| P1 | 6 | 조회 API 3건 · Advisor 계약 2건 · role 규정 1건 | **6건 처리** |
| P2 | 6 | Advisor 나머지 3건 · §22 미확정 3건 | 3건 처리 · 3건은 §22 확정 대상 |
| P3 | 3 | ~~보존 정책~~ · 신원 결합 · `message` 언어 | 2건 처리 · **1건 철회**(A-20 은 §22-7 이 이미 덮는다 — 제 오인) |

**2026-08-30 처리 결과 — `etl-platform-target-architecture-v1.2.4.md`.**
13건 반영(D-01 · A-8 · A-9 · A-10 · A-11 · A-12 · A-13 · A-15 · A-17 · A-18 · A-19 ·
A-21 · A-24), **1건 철회**(A-20), 3건은 A §22 확정 대상으로 남긴다(A-14 · A-22 · A-23).
각 건의 반영 위치는 v1.2.4 머리말의 "변경 근거(v1.2.4)" 줄과 본문의 `v1.2.4` 표기를 보라.
...
> **결정(2026-08-30) — ① 을 택했다.** 인가 모델을 미루기로 한 이상 ① 의 가장 큰 부담
> (권한을 DB role 로 거는 것)이 지금은 없고, Phase 0 구현량이 가장 작다.
> v1.2.4 §6.1 「운영자 조회 표면」이 뷰 목록·권한·예외 2건을 규정한다.
...
### ~~A-20. 실행 이력의 보존 기간 정책이 없다~~ — **철회(2026-08-30). 제 오인이었다**

**이 건은 공백이 아니다.** A §22-7 이 이미 규정한다.

> 7. Run/Event/log/Snapshot 보관 기간 — Full 테이블 snapshot retention 등급,
> CommitEvidenceLedger 보관 기간, **6.1 이력·계측 테이블 보관 기간**(history 3종·
> `guard_result` = ledger와 동일, **`attempt_timeline` 온라인 90일 잠정** — 5.3) 포함

항목이 있을 뿐 아니라 **잠정값까지 있다.** 이 요청서 초판은 §18(Iceberg maintenance)만
보고 "Control DB 쪽은 없다"고 썼는데, §22-7 을 확인하지 않은 것이다.

**남은 것은 공백이 아니라 소비처 명시뿐이다.** v1.2.4 는 §22-7 에 한 줄을 더해
**운영자 UI 의 실행 이력 화면이 덮는 구간이 이 값에서 나온다**고 적었다. 새 항목이
아니다.

> 이 철회는 이 요청서 §0 이 스스로 요구한 규율 — **"A 에 없다"고 쓰기 전에 다른 절을
> 본다**(§12.0 의 교훈) — 을 정작 이 항목에서 지키지 못한 사례다. 남겨 둔다.
