...
도달 불가로 보이지만 **아직 측정하지 않았다.** 제약의 정확한 표현은 이렇다(2026-08-27 통일 — 7차 리뷰 P1-12) — **정합성을 DBA 협조에 걸 수 없다. 비critical 권한 요청은 가능하나 현재 보류이며 절대 가정하지 않는다.** 그러므로 `FLASHBACK` 객체 권한이 살아날 여지는 있으나 **설계가 그것을 전제해서는 안 된다.** 지금 지우면 되살릴 때 되살릴 것을 잃는다.
