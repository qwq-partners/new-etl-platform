...
| **`etl-platform-a-revision-request-ui.md`** | **A 개정 요청서 17건** — UI 정보구조를 쓰다 발견한 A 자체의 공백. **P0 둘**: 조회 경로 선택(D-01)과 **`*_at` 필드 25개의 저장 시간대 미선언**(A-13 — UI 와 무관하게 A 의 계산식이 성립하지 않을 수 있다). 나머지는 P1~P3 |
