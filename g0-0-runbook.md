...
**게이트는 지금 닫혀 있다**(9차 조치 9 — `g0_final_gate.GATE_OPEN = False`). G0-0 레코드만이
아니라 **어떤 레코드도** 승인되지 않는다. G0-1~G0-5 산출물이 하나도 없고 `aggregate()` 가
구현되지 않았으므로 들여보낼 곳 자체가 없다. 여는 조건 넷은 그 상수의 주석에 있다.

항목의 위치는 계약의 `where` 만이 말한다. `item` 은 식별자이고 `label` 은 표시 이름이다 —
이전 판은 `item` 을 경로로 해석해서, 계약이 표시용으로 적어 둔 이름과 같은 키 하나만 있으면
그 항목이 충족된 것으로 읽었다. 전부 쓰레기 문자열인 레코드가 그렇게 승인됐다(9차 §5-1).
`where` 가 null 인 항목은 **위치가 아직 정해지지 않았다**는 뜻이고 어떤 레코드로도 충족되지
않는다. 그 위치는 G0-1~G0-5 실물이 생길 때 정한다.
