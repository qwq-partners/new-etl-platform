# ETL 관리 UI — 정보구조 (Phase 0)

> **이 문서는 현행 하나뿐이다. 판본 사본을 뜨지 않고 여기를 직접 고친다**(2026-08-30 결정).
> 변경 이력은 git 이 남긴다 — 예전 `…-v0.1.md` ~ `-v0.6.md` 는 삭제했고 이력에서 볼 수 있다.
> 새 결정이 생기면 아래 표에 한 줄을 더하고 본문을 고친다. **새 파일을 만들지 않는다.**

## 결정 기록

| 날짜 | 결정 | 본문 |
|---|---|---|
| 2026-08-30 | 문서 골격 — 화면 8개, Phase 0 표시 규칙 넷, 두 UI 접합면 | §3 · §4 · §5 · §6 |
| 2026-08-30 | **Advisor UX** — 입력 칸을 채우지 않는다 · 거절이 1급 동작 · Source 보호가 추천을 이긴다 | §10 |
| 2026-08-30 | **시각 표시** — 고정 `Asia/Seoul` + 오프셋 라벨 · window 를 시각으로 그리지 않는다 · 시계 셋을 섞지 않는다 | §11 |
| 2026-08-30 | **운영자 조회** — 경로 규칙 5개 · `etl_ui` 뷰 계층 제안 | §12 |
| 2026-08-30 | **목록 규모** — 커서 페이징만 · 총 건수 미반환 · 대시보드 상한 | §13 |
| 2026-08-30 | **인가는 미루고 인증·감사는 하지 않을 수 없다** — 운영팀 전용, ID/PW → 추후 사내 SSO | §14 |
| 2026-08-30 | **계정 ID = 사번** — Dagster proxy 로그와 잇는 키 | §14.3 |
| 2026-08-30 | **다국어 하지 않는다** — 단, rule 이름·enum 은 번역하지 않는다 | §11.4 |

**남은 미결은 하나다 — §9-6 capability 표시 형태.** UI 가 정할 수 있는 것이 아니라
G0-0 실측이 답한다. 그 밖에 **A 개정이 필요한 공백 17건**은 §9.1 에 따로 세워 뒀다.
...
> **정정(2026-08-30).** 이 표에 처음에는 "Guard 거부 **추이**"라고 썼는데 바로 아래 줄에서
...
| 블록 | 내용 |
|---|---|
| 계정 | **1인 1계정** 목록·생성·비활성. **ID 는 사번이고 화면이 형식을 강제·정규화한다**(§14.3). 공용 계정을 만들 수 없어야 하고(§14.2-1), **계정을 삭제할 수 없어야 한다** — 비활성만(§14.3-3) |
...
| **2** | ③ Job Wizard 1·2·4·6·7·10·11·13 단계 | **5(cutoff)·3(Advisor)·12(profile)는 Phase 0 에서 뺀다** — 5는 §3.1 때문에 선택지가 하나뿐이고, 12는 튜닝 결과가 있어야 의미가 있으며, **3은 A §15.4 가 auto-prefill 앞에 Shadow 평가·측정을 두기 때문이다**(§10.7 — 처음에는 "측정·튜닝 결과가 있어야 의미가 있다"고 12단계와 묶어 이유를 흐리게 썼다) |
...
**Phase 0 이 조회에서 지켜야 할 것**(§12·§13). 화면 4개뿐이어도 이 넷은 처음부터
...
- **A 를 인용할 때 어느 절인지 확인하고 쓴다.** §9-1 의 전제가 뒤집힌 것은 §17 만 보고
...
- ~~**권한 모델이 A 에 없다.**~~ **결정으로 닫았다**(§14) — 인가는 미루고
  ...
  **계정 ID = 사번으로 정했다**(§14.3). 다만 그것은 강제가 아니라 규율이고,
  **OIDC 의 `sub` 가 사번이라는 보장이 없어** §14.4 의 확인 셋이 남아 있다
- ~~**10,000 Job·40,000 Run 규모에서 이 정보구조가 서는지 모른다.**~~ **구조를 정했다(§13).**
...
| # | 미결 | 왜 지금 정하지 않는가 |
|---|---|---|
| 1 | ~~**운영자용 조회 API**~~ | **해소 → §12.** 화면 21건에서 역으로 도출, 조회 경로 규칙 5개, `etl_ui` 뷰 계층 제안. **초기 판의 전제("A 개정이다")를 정정했다** — A 는 §16.2 에서 read replica 로 이미 답했고, 다만 그 한 문장이 21건 중 6건만 덮는다(§12.0·§12.3). A 가 골라야 할 것은 §12.6 |
| 2 | ~~**권한·승인 모델**~~ | **결정으로 닫았다 → §14.** 인가는 미룬다(운영팀 전용·전원 동일 권한). **인증·감사는 미루지 않는다** — A 가 `AuditEvent.actor`·`auth_method` 를 필수로, `사유·승인자 필수`를 7곳에서 요구한다. **계정 ID = 사번**(§14.3). 남은 것은 미결이 아니라 **확인 사항 셋**이다(§14.4) |
| 3 | ~~**목록 규모 대응**~~ | **해소 → §13.** 커서 페이징·총 건수·기본 필터·N+1·대시보드 상한·정렬 키. **구조는 정했고 값 5개는 G1 이 답한다**(§13.7) |
| 4 | ~~**Advisor UX**~~ | **해소 → §10.** 화면 규칙 7절. A 가 답해야 할 잔여는 §10.8 |
| 5 | ~~**시각(timezone) 표시**~~ | **해소 → §11.** 표시 규칙 4개. **다국어는 분리해 아래 7번으로 남긴다** |
| **6** | **capability 표시 형태** ← **남은 유일한 미결** | §3.2 로 Phase 0 은 막았지만, G0-0 이후 무엇을 어떻게 보여 줄지는 축이 확정된 뒤에. **UI 가 정할 수 있는 것이 아니라 측정이 답한다** |
| **7** | ~~**다국어**~~ | **결정으로 닫았다 → §11.4.** 하지 않는다. **다만 rule 이름·enum·거부 사유 코드를 번역하지 않는다는 규칙은 남는다** — 그것은 다국어 규칙이 아니라 §3.4 규칙이고, 단일 언어 UI 에서 오히려 더 위험하다 |

### 9.1 미결을 채우다 새로 드러난 것 — 전부 A 쪽 공백이다 — 전부 A 개정이 필요하다

미결 1·2·3·4·5·7 을 채우려고 A 를 정독한 결과, **A 자체의 공백이 열일곱** 나왔다. 이것들은
...
| **21** | **Control UI 신원과 Dagster proxy 신원을 잇는 키가 A 에 없다.** A §10.2 는 `OPERATOR_CANCELLED` 의 actor 를 proxy access log 로 보강한다고 하는데, 그 log 의 주체와 `AuditEvent.actor` 가 같은 사람이라는 것을 말할 방법이 없다. **사번으로 잇기로 했다**(§14.3) — A 개정 없이 규율로 막는다. 다만 §14.4 의 확인 셋이 전제다 | §14.3 |
| **22** | A §22-10 **"Dagster UI 노출 정책"이 A 자신의 미확정 항목이다** — read-only 분리·mutation allowlist·감사 | §14.5 |
| **23** | A §22-13 **"Control API 역할 모델·Run Pod 신원"도 미확정이다.** 다만 같은 줄의 "2인 승인/SoD는 정책 선택사항"이 인가를 미루는 결정과 어긋나지 않는다는 근거다 | §14.5 |
| **24** | **`violations[].message` 의 언어가 A 에 규정돼 있지 않다**(경미). §3.4 는 그것을 그대로 보여 주라고 하므로 **UI 가 고칠 수 없다** — API 가 영문 message 를 주면 한국어 화면에 영문 문장이 섞인다. `rule_id` 는 원문 유지가 규칙이지만(§11.4) `message` 는 설명 문장이라 다르다 | §11.4 |

**그리고 초기 판의 전제 둘을 정정했다.**

1. 5번에 "계약 시각은 UTC 다" 라고 쓰여 있었는데 **A 에 그런 규정이 없다**(§11.0).
   표시 시간대를 고르는 것은 UI 결정이지만 **저장 시간대 확정은 A 개정**이다.
2. 1번에 조회 API 도출이 **"A 개정이다"** 라고만 쓰여 있었는데, **A 는 §16.2 에서
...
| 21 | **규율로 막았다** — 계정 ID = 사번(§14.3). A 개정 불필요. **§14.4 의 확인 셋이 남았다** |
| 22 · 23 | **A §22-10 · §22-13 확정** |
| 24 | **A §17 개정**(한 줄) 또는 Control API 구현 시 한국어로 고정 |

**8~24 를 UI 문서가 채울 수 없다.** 채우면 규범에 없는 것을 지어내는 것이고, 이 저장소가
여덟 차례 리뷰에서 반복해 지적당한 바로 그 결함이다.

### 9.3 미결 1·2·3 은 하나였다 — 그리고 §14 결정이 그 사슬을 끊었다

조회 API 를 도출하다 이 사슬이 드러났다.
...
### 10.7 Phase 0 에는 이 화면이 없다 — 처음의 이유를 정정한다

§7 은 Wizard 3단계를 Phase 0 범위에서 빼면서, 처음에는 이유를 12단계와 묶어 "측정·튜닝 결과가
...
| **Advisor 가 독립 화면인가 Wizard 단계인가** | A §16.1 은 Custom UI 목록에서 `Advisor` 를 `Source`·`Job Wizard/CRUD` 와 **나란한 항목**으로 적어 독립 화면을 시사하는데, A §7.2 는 Wizard 3단계로만 둔다. 이 문서는 Wizard 단계로 읽었다 — **A 안에서 두 절이 어긋난다** |

---

## 11. 시각 표시 — §9-5 해소 (시각 부분)

### 11.0 먼저, 초기 판의 전제를 정정한다
...
**따라서 "이 문서 안에서 정할 수 있다"는 절반만 맞았다.** 표시 시간대를 고르는
...
### 11.4 다국어 — 하지 않는다 (결정, 2026-08-30)

> **다국어 필요 없어.**

§9-5 는 원래 "다국어·시각(timezone) 표시"를 한 항목으로 묶고 있었고, 뒤에 둘을 떼어
다국어를 §9-7 로 남겼다. **이제 닫는다 — UI 는 한국어 단일 언어다.**

**그래서 만들지 않는 것**

- i18n 프레임워크 · 메시지 카탈로그 · 로케일 전환 UI. **"나중을 위해" 카탈로그만 만들어
  두지 않는다** — 쓰이지 않는 간접층은 문자열을 찾기 어렵게만 만든다
- `Accept-Language` 처리 · 언어별 자산 분기

화면 문구는 **한국어 문자열을 코드에 그대로** 쓴다.

**그런데 §11.4 의 규칙은 남는다 — 그것은 다국어 규칙이 아니었다.**

> **rule 이름 · 상태 enum · 거부 사유 코드는 번역하지 않는다.**

단일 언어 UI 에서 오히려 더 위험하다. 화면 전체가 한국어면 `SOURCE_LAG_EXCEEDED` 하나만
영문으로 남는 것이 어색해 보여 "원천 지연 초과"로 바꾸고 싶어진다. **그러면 운영자가 A
문서에서 그 사유를 찾을 수 없다.** §3.4 가 요구하는 것이 정확히 이것이고, 번역은 감추는
것의 한 형태다.

- `SOURCE_LAG_EXCEEDED` · `ADJUDICATION_PENDING` · `DIRECT_LAUNCH_FORBIDDEN` 은 **원문 그대로**
- 한국어 설명은 **원문 옆에 붙이지, 원문을 대체하지 않는다**
- 표·목록의 열 값에도 같다. 상태 열에 한국어만 있으면 그 화면은 A 와 대조할 수 없다

**하드코딩해도 되는 것과 안 되는 것**

| | 예 | 규칙 |
|---|---|---|
| UI 자체 문구 | 버튼·머리글·안내문 | **한국어로 하드코딩한다.** 다국어가 없으므로 간접층이 필요 없다 |
| **서버가 주는 식별자** | `violations[].rule` · 상태 enum · Guard 사유 · **capability 축 이름** | **하드코딩도 매핑도 하지 않는다.** 서버가 주는 것을 그대로 렌더한다(§3.2 의 축 이름 규칙이 여기의 일반형이다) |

> 숫자·시각 형식은 로케일 문제가 아니다. **§11.1~11.3 의 규칙이 권위이며**, 다국어를
> 하지 않는다고 해서 그 규칙이 느슨해지지 않는다 — 오프셋 라벨은 여전히 필수다(규칙 1).
...
**확정**(§14 결정 반영): 인가 모델을 미루기로 했으므로 "내 부서"를 화면이 알 방법이
...
답을 낸다. §5-⑤ 1항이 요구하는 **"effective mode 와 그것을 만든 Hold 목록을 함께"** 는
계산이 한 곳일 때만 지켜진다.
...
> 대시보드에 페이징이 생기는 순간 그 화면은 목적을 잃는다. §5-① 이 "사람이 개입해야 할
...
### 14.3 두 UI 의 신원을 잇는다 — **ID 는 사번이다** (결정)
...
**Control UI 가 자기 계정 체계를 가지면 같은 사람이 두 신원을 갖고 잇는 키가 없다.**

> §6 은 "같은 것을 두 곳에서 편집할 수 있게 만들지 않는다"고 했다. **신원은 그
> 규칙 밖에 있었다** — 편집 대상이 아니라 편집하는 사람이기 때문이다.

**그리고 A 는 사람 인증을 이미 OIDC 로 못박아 뒀다.** §17 마지막 줄:

> 인증·역할 모델(**사람: 사내 SSO OIDC**, 자동화: 서비스 계정, Run Pod: projected
> ServiceAccount token + TokenReview + contract-scoped 단기 토큰)은 **22장 13번에서 확정한다**

즉 "추후 사내 SSO"는 **A 의 계획과 같은 것**이다. 사번을 ID 로 쓰는 결정은 A 와 어긋나지
않는다 — **다만 조건이 하나 붙는다**(§14.4).
...
**(1) `AuditEvent.actor` 에 사번 문자열을 그대로 넣는다.**
내부 계정 UUID 를 넣지 않는다. UUID 를 넣으면 Dagster proxy 로그와 조인할 수 없고,
그러면 이 결정을 한 이유 자체가 없어진다. UUID 가 필요하면 **계정 테이블의 PK 로만** 쓰고
`actor` 에는 사번을 쓴다.

**(2) 계정 생성 화면이 사번 형식을 강제하고 정규화한다.**
자유 문자열이면 오타·별칭이 섞이고 그 순간 조인이 깨진다. 형식은 사내 규칙을 따르되
화면이 **대소문자·공백·선행 0 을 정규화해서** 저장한다. 정규화하지 않으면 `0012345` 와
`12345` 가 다른 사람이 된다.

**(3) 계정을 삭제하지 않는다. 비활성만 한다.**
`AuditEvent.actor` 는 문자열이라 계정 row 가 없어도 기록 자체는 남는다. 그러나 화면이
"이 사번이 누구였는가"를 보여 주려면 row 가 있어야 한다. **퇴사자 계정을 지우면 과거
감사 기록이 사번만 남은 익명 기록이 된다.**

**(4) 사번 재사용을 전제하지 않는다.**
사번이 재사용되면 과거 기록이 **다른 사람에게 붙는다.** 사내 인사 규칙이 재사용을
허용하면 `(사번, 재직 구간)` 을 키로 써야 한다. **확인할 사항이지 가정할 사항이 아니다**(§14.4).

### 14.4 이 결정에 붙는 조건 — IdP 가 사번을 주는가

결정이 성립하는 근거는 **"지금 쓰는 ID 문자열 = SSO 가 나중에 줄 값"** 이다.
**OIDC 에서 그것이 자동으로 성립하지 않는다.**

- OIDC 의 **`sub` 는 보통 불투명 식별자**다. IdP 내부 키이지 사번이 아니다
- 사번은 대개 **별도 claim**(`employeeNumber`·`employeeId`·사내 확장 claim 등)으로 오고,
  **IdP 설정에 따라 아예 안 올 수도 있다**
- Dagster 앞 reverse proxy 가 access log 에 남기는 주체도 마찬가지다 — 보통 `sub` 또는
  이메일이지 사번이 아니다

**그래서 지금 확인할 것 셋.** SSO 도입 시점이 아니라 **지금**이다 — 그때 가서 알면
이미 감사 기록이 쌓인 뒤다.

| 확인 | 왜 |
|---|---|
| 사내 IdP 가 **사번을 claim 으로 발급하는가**, claim 이름이 무엇인가 | 안 주면 이 결정이 성립하지 않고 매핑표가 돌아온다 |
| **Dagster 앞 proxy 가 access log 에 무엇을 남기는가**(그리고 사번을 남기게 설정할 수 있는가) | 사번을 남길 수 있어야 A §10.2 의 actor 보강이 Control 기록과 이어진다 |
| 사내 인사 규칙이 **사번을 재사용하는가** | 재사용하면 §14.3-(4) |

**이 셋이 확인되기 전까지 사번 결정은 "비용 0 의 보험"이지 "보장"이 아니다.**

확인 결과가 나빠도 대안은 남는다 — SSO 도입 시 `AuditEvent` 에 `external_subject` 를
더해 `사번 ↔ sub` 를 잇는다. **다만 그것은 과거 기록에 소급되지 않으므로, 지금 사번을
쓰는 것이 어느 경우에도 손해가 아니다.** 그래서 확인 전에 시작해도 된다.
...
| SSO 프로토콜 세부 | A 가 **OIDC** 라고만 정했고 나머지는 사내 표준이 정해져야 한다 |
| **A §22-13** | A 자신의 미확정 항목 — "Control API 역할 모델·Run Pod 신원. **2인 승인/SoD는 정책 선택사항**". 마지막 구절이 인가를 미루는 이번 결정과 어긋나지 않는다는 근거다 |
| **A §22-10** | 같은 미확정 — "Dagster UI 노출 정책: read-only 인스턴스 분리, write 인스턴스의 SSO proxy·mutation allowlist·감사". 확정되면 §14.3·§14.4 를 재검토한다 |
