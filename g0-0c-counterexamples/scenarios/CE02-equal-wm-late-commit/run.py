...
def cycle(res, conn, src, cur_t, sink, job, no, eligible_only=False):
    ...
    res.cas_at = server_now(conn)          # 서버 시각. 뒤에서 late commit 시각과 대조한다.
    conn.commit()
    if moved != 1:
...
def body(res, ora):
    ...
    with Fixture(res, reader, ora) as fx:
        ...
        cas_at = getattr(res, "cas_at", None)
        res.obs("cycle1_cas_server_time", cas_at)
        ...
        if after == before + 1:
            ...
            # TIMING 은 **서버 시각끼리** 비교했을 때만 증거다. 회차1 CAS 시각과 대조한다.
            if cas_at and commit_at > cas_at:
                res.evidence("TIMING",
                             f"회차1 CAS 서버 시각 {cas_at} < late commit 서버 시각 {commit_at} — "
                             "봉인 이후에 commit 됐다.", at=commit_at)
        else:
            res.obs("injection_failed", {"before": before, "after": after},
                    note="독립 세션이 센 행 수가 변하지 않았다. 주입이 서버에 반영되지 않았다.")
        ...
        res.obs("recovered_by_source_progress", 999 in c3,
                expected=False, matches=(999 not in c3),
                note="source progress 가 재개된 회차3 의 적재 목록에 999 가 있는지 실제로 본다.")

        if exists_999 != 1:
            # 주입한 행이 원천에 없다 = late commit 자체가 안 일어났다. 판정하지 않는다.
            res.outcome = OUTCOME_NOT_OBSERVED
            res.obs("verdict_note",
                    f"주입 행 999 가 원천에 {exists_999}건이다 — late commit 이 반영되지 않았다. "
                    "봉인 성립 여부를 판정할 수 없다.")
        elif loaded_999 == 0:
