...
| `suite_verdict.execution_complete` / `.mitigation_holds` | **`pass` 하나로 두 가지를 말하고 있었다**(P2). `COUNTEREXAMPLE_REPRODUCED`·`MITIGATION_FAIL` 도 `pass=true` 에 포함된다 — 하네스 완주로는 맞지만 **설계 통과로 오독된다.** 이제 `execution_complete`(하네스가 돌았는가)와 `mitigation_holds`(설계가 버텼는가)를 따로 낸다. 재현된 반례·완화 실패 목록도 함께 남긴다 |
