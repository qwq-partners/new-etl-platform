# ETL 관리 UI — 정보구조 v0.5 (Phase 0)

> **v0.4 대비 변경**(2026-08-30). v0.4 §14.3 이 안 셋을 놓고 (a) 를 권했고, 사용자가 골랐다.
>
> > **ID 는 사번으로 맞추자, (a) 로 가**
>
> | 무엇 | 어디 |
> |---|---|
> | **Control UI 계정 ID = 사번** — 권고에서 결정으로. 규칙 넷(`actor` 에 사번 문자열 그대로 · 형식 강제와 정규화 · 계정 삭제 금지 · 사번 재사용 비전제) | §14.3 |
> | **결정에 붙는 조건** — OIDC 의 `sub` 는 보통 불투명 식별자다. **사내 IdP 가 사번을 claim 으로 주는지 지금 확인해야** 이 결정이 성립한다 | **§14.4 신설** |
> | A §17 이 사람 인증을 **사내 SSO OIDC** 로 이미 못박아 뒀다 — "추후 SSO"는 A 의 계획과 같은 것 | §14.3 |
> | A §22-13("2인 승인/SoD는 정책 선택사항")이 인가를 미루는 결정과 어긋나지 않는다는 근거 | §14.5 |
>
> **v0.1~v0.4 는 이력으로 남긴다. 이 문서가 현행이다.**
>
> ---
>
...
미결 1·2·3·4·5 를 채우려고 A 를 정독한 결과, **A 자체의 공백이 열여섯** 나왔다. 이것들은
...
**8~23 을 UI 문서가 채울 수 없다.** 채우면 규범에 없는 것을 지어내는 것이고, 이 저장소가
