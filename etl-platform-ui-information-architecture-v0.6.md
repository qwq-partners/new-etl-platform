# ETL 관리 UI — 정보구조 v0.6 (Phase 0)

> **v0.5 대비 변경**(2026-08-30). 남은 미결 둘 중 **7(다국어)** 을 사용자 결정으로 닫았다.
>
> > **다국어 필요 없어.**
>
> | 무엇 | 어디 |
> |---|---|
> | **UI 는 한국어 단일 언어.** i18n 프레임워크·메시지 카탈로그·로케일 전환을 만들지 않는다 — **"나중을 위해" 카탈로그만 만들어 두는 것도 안 한다** | §11.4 |
> | **그래도 rule 이름·enum·거부 사유 코드는 번역하지 않는다** — 이건 다국어 규칙이 아니라 §3.4 규칙이고, **단일 언어 UI 에서 오히려 더 위험하다** | §11.4 |
> | 하드코딩해도 되는 것(UI 문구)과 안 되는 것(서버가 주는 식별자)의 구분 | §11.4 |
> | `violations[].message` 의 **언어가 A 에 없다** — §3.4 가 그대로 보여 주라 하므로 UI 가 고칠 수 없다 | §9.1-24 |
>
> **남은 미결은 하나다 — 6(capability 표시 형태). G0-0 실측이 답한다.**
>
> **v0.1~v0.5 는 이력으로 남긴다. 이 문서가 현행이다.**
>
> ---
>
> **v0.5 가 채운 것** — §14.3 계정 ID = 사번, §14.4 그 결정에 붙는 조건(IdP 가 사번을 주는가).
>
> ---
>
> **v0.4 가 채운 것**(v0.1 §9 의 미결 2) — §14 인증·감사.
> 인가는 미루되 인증·감사는 A 가 요구하므로 미루지 않는다.
>
> ---
>
...
- ~~**권한 모델이 A 에 없다.**~~ **v0.4 에서 결정으로 닫았다**(§14) — 인가는 미루고
  ...
  **v0.5 에서 계정 ID = 사번으로 정했다**(§14.3). 다만 그것은 강제가 아니라 규율이고,
...
| 2 | ~~**권한·승인 모델**~~ | **v0.4·v0.5 에서 결정으로 닫았다 → §14.** 인가는 미룬다(운영팀 전용·전원 동일 권한). **인증·감사는 미루지 않는다** — A 가 `AuditEvent.actor`·`auth_method` 를 필수로, `사유·승인자 필수`를 7곳에서 요구한다. **계정 ID = 사번**(§14.3). 남은 것은 미결이 아니라 **확인 사항 셋**이다(§14.4) |
...
| **7** | ~~**다국어**~~ | **v0.6 에서 결정으로 닫았다 → §11.4.** 하지 않는다. **다만 rule 이름·enum·거부 사유 코드를 번역하지 않는다는 규칙은 남는다** — 그것은 다국어 규칙이 아니라 §3.4 규칙이고, 단일 언어 UI 에서 오히려 더 위험하다 |
...
| **21** | **Control UI 신원과 Dagster proxy 신원을 잇는 키가 A 에 없다.** A §10.2 는 `OPERATOR_CANCELLED` 의 actor 를 proxy access log 로 보강한다고 하는데, 그 log 의 주체와 `AuditEvent.actor` 가 같은 사람이라는 것을 말할 방법이 없다. **v0.5 에서 사번으로 잇기로 했다**(§14.3) — A 개정 없이 규율로 막는다. 다만 §14.4 의 확인 셋이 전제다 | §14.3 |
...
| 21 | **v0.5 에서 규율로 막았다** — 계정 ID = 사번(§14.3). A 개정 불필요. **§14.4 의 확인 셋이 남았다** |
...
v0.1 §9-5 는 "다국어·시각(timezone) 표시"를 한 항목으로 묶었고, v0.2 가 둘을 떼어
