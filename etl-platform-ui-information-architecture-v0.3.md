# ETL 관리 UI — 정보구조 v0.3 (Phase 0)

> **v0.2 대비 변경**(2026-08-30). v0.2 §9 미결 중 **1(운영자 조회)·3(목록 규모)** 를 채웠다.
>
> | 무엇 | 어디 |
> |---|---|
> | 운영자 조회 — 화면 21건에서 역으로 도출, 조회 경로 규칙 5개, `etl_ui` 뷰 계층 제안 | **§12 신설** |
> | 목록 규모 — 커서 페이징·총 건수·기본 필터·N+1·대시보드 상한·정렬 키 | **§13 신설** |
> | **v0.1·v0.2 §9-1 의 전제를 정정** — A 는 §17 이 아니라 **§16.2 에서 이미 답했다**(read replica) | §12.0 |
> | §5-① "Guard 거부 **추이**" → **현재 건수**로 정정. 추이는 Grafana 다 | §5-① |
> | §9-1·§9-2·§9-3 이 **하나로 묶인다**는 것을 확인 — 셋 중 하나만 풀 수 없다 | §13.3 |
> | A 개정 제안 3건 추가 | §12.6 |
>
> **v0.1·v0.2 는 이력으로 남긴다. 이 문서가 현행이다.**
>
> ---
>
...
| 2 | **권한·승인 모델** | `FORCE_STOP`·`dq:accept`·`force revoke`·`accept_row_drop` 이 "승인 필요"라고만 돼 있고 **누가** 승인하는지가 없다. **§13.3 에서 확인 — 이것 없이는 Job 목록의 기본 필터도 정할 수 없다** |
...
### 9.1 v0.2·v0.3 을 쓰다 새로 드러난 미결 — 전부 A 개정이 필요하다
...
| 미결 | 푸는 것 |
|---|---|
| 2 | **A 개정 + 조직 결정** — 누가 승인하는가. **§13.3 이 확인했듯 이것 없이는 Job 목록 기본 필터도 못 정한다** |
...
**8~20 을 UI 문서가 채울 수 없다.** 채우면 규범에 없는 것을 지어내는 것이고, 이 저장소가
여덟 차례 리뷰에서 반복해 지적당한 바로 그 결함이다.

### 9.3 미결 1·2·3 은 하나다

v0.3 을 쓰면서 확인한 것 — **§9-1(조회)·§9-2(권한)·§9-3(규모)는 따로 풀 수 없다.**
...
셋 중 **가장 위가 §12.6-a 다.** 그것을 A 가 고르기 전에는 아래 둘이 잠정으로만 선다.
Phase 0 은 그 잠정으로 간다 — §13.3 의 "상태가 정상이 아닌 것만" 기본 필터.
...
Phase 0 잠정: 권한 모델 전까지는 **"최근 상태가 정상이 아닌 것"만** 기본으로 걸고
부서 필터는 수동 선택으로 둔다.
