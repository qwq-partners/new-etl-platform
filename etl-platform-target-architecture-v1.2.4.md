# 신규 ETL Platform 목표 아키텍처 v1.2.4

- 문서 상태: PoC 승인 후보안 — v1.2.4 (**UI 정보구조에서 도출한 A 개정 요청 처리** — 규정 공백을 메우고 두 절의 불일치를 정합. 새 상태·새 실행 경로 0)
- 작성일: 2026-08-22 (v1) · 개정일: 2026-08-23 (v1.1) · 2026-08-23 (v1.2) · 2026-08-23 (v1.2.1) · 2026-08-23 (v1.2.2) · 2026-08-23 (v1.2.3) · 2026-08-24 (v1.2.3.1) · 2026-08-30 (v1.2.4)
...
- 변경 근거(v1.2.4): `etl-platform-a-revision-request-ui.md` — Control Plane UI 정보구조를 쓰며 A 를 정독해 발견한 규정 공백 17건 중 **13건 처리**(D-01·A-8·A-9·A-10·A-11·A-12·A-13·A-15·A-17·A-18·A-19·A-21·A-24). **가장 중요한 것은 A-13** — 25개 넘는 시각 컬럼의 저장 타입·시간대가 선언돼 있지 않은데 A 는 그 값들을 서로 다른 세 시계 사이에서 뺀다(16.4 lateness, 10.2 undo deadline). naive 저장에서는 그 비교가 조용히 틀리고 undo deadline 은 fence 만료를 놓친다. 나머지는 조회 표면 결정(D-01 — replica + `etl_ui` 뷰), Advisor 계약 확정, 판정 직전 재확인 API, 신원 결합. **미처리 4건**: A-14(22장 12번 — Iceberg 타입만 남음)·A-20(22장 7번이 이미 덮음 — 요청서 쪽 오인)·A-22(22장 10번)·A-23(22장 13번)은 전부 22장 확정 대상이다
...
- 공통 `AuditEvent` (actor, auth_method, source_ip, idempotency_key 필수), `IdempotencyRecord`
  - **`actor` 는 사번 문자열이다**(v1.2.4 신설 — A-21). 내부 계정 UUID 를 넣지 않는다. 4장이 Dagster write 인스턴스를 SSO reverse proxy 뒤에 두고 **"proxy access log 가 감사 기록"** 이라 규정하며 10.2 표가 `OPERATOR_CANCELLED` 의 actor 를 그 로그로 보강하는데, Control 이 자기 계정 체계를 쓰면 **같은 사람이 두 신원을 갖고 잇는 키가 없다.** 양쪽이 같은 사번을 쓰면 한 사건을 두 로그로 추적할 수 있다.
  - 계정은 **1인 1계정**이고 **삭제하지 않는다**(비활성만). `actor` 는 문자열이라 계정 row 가 없어도 기록은 남지만, "이 사번이 누구였는가"를 답하려면 row 가 있어야 한다 — 퇴사자 계정을 지우면 과거 감사 기록이 익명 기록이 된다. 사번 재사용은 전제하지 않는다(재사용하는 인사 규칙이면 `(사번, 재직 구간)` 이 키여야 한다).
  - **`auth_method` 는 인증 방식 교체를 전제한 필드다.** 사람 인증은 22장 13번이 사내 SSO OIDC 로 확정하되 그 전 단계의 ID/PW 도 같은 필드에 기록한다 — 값을 나중에 넣으면 전환 시점을 경계로 감사 기록이 갈라진다. **OIDC 의 `sub` 는 보통 불투명 식별자이므로**(사번은 별도 claim) SSO 도입 시 그 claim 을 `actor` 에 싣는 매핑을 22장 13번이 함께 확정한다.
...
#### 시각 컬럼 규약 (v1.2.4 신설 소절 — A-13)

**규칙.** 6.1 의 모든 시각 컬럼(`*_at` · `*_ts`)은 **`TIMESTAMP WITH TIME ZONE`(`timestamptz`)이고 값은 UTC 로 저장한다.** 예외는 `execution_contract.window_range`(13.4 가 따로 규정 — `APPLICATION_TIMESTAMP` 는 UTC epoch 마이크로초, `STANDBY_VISIBLE_SCN` 은 SCN 정수이며 시각이 아니다) 하나뿐이다.

대상은 다음 전부다 — `created_at` · `next_eligible_at` · `expires_at` · `submitted_at` · `submission_in_flight.requested_at` · `first_guard_ok_at` · `last_cas_at` · `finalized_at` · `fence_ts` · `deployed_at` · `effective_from` · `reattach_deadline` · `terminal_ingested_at` · 이력 3종의 `at` · `guard_result.run_started_at`·`at` · `attempt_timeline.t0_queued_at`~`t7_finalized_at`·`resubmit_t0[]` · `cas_at` · `last_reconcile_at` · `verified_at`(3종 evidence blob) · `cap_plus_one_verified_at` · `released_at` · `hold_release_at` · `publish_at` · `last_expected_checked_at` · `logical_scheduled_at`.

**`logical_scheduled_at` 의 이름.** 9.2 의 자연키 제약이 `logical_scheduled_at_utc` 로 쓰고 본문 여섯 곳이 접미사 없이 쓴다. **둘은 같은 컬럼이고 이 규약이 그것을 UTC 로 확정한다.** 표기는 `logical_scheduled_at` 으로 통일하되 9.2 제약 이름은 이미 배포된 DDL 식별자이므로 바꾸지 않는다.

**왜 규정하는가 — 이 값들을 실제로 뺀다.** A 는 이미 세 시계를 쓴다.

| 시계 | 값 |
|---|---|
| **primary** | `T_lb`(heartbeat, 11.3) · 원천 `UPDATE_DT`·watermark 컬럼 |
| **standby** | `fence_ts` · `SYSTIMESTAMP_standby`(11.3 cutoff) |
| **Control 호스트** | 위 목록의 나머지 전부 (`now()` 기준) |

그리고 아래 넷이 이 값들 사이의 뺄셈·비교다.

- `lateness(o) = t7 − o.logical_scheduled_at` (16.4)
- `now() − logical_scheduled_at > freshness_slo` (16.4 breach 조건 (a))
- **`now() + 예상 chunk 소요 > contract.fence_ts + undo_retention_seconds × 0.5`** (10.2 `chunks:begin` undo deadline)
- `now() − max(coalesce(ledger.cas_at, contract.finalized_at))` (16.2 target publication age)

PostgreSQL 의 `now()` 는 `timestamptz` 다. 컬럼이 naive `timestamp` 면 **세션 `TimeZone` 이 저장값을 정하고**, Control API·5.4 scheduler·Run Pod 가 서로 다른 `TimeZone` 으로 뜨면 위 비교가 조용히 틀린다. `freshness breach` 가 오프셋만큼 이르거나 늦게 뜨고, **undo deadline 검사는 fence 만료를 놓친다** — 그것은 알림 문제가 아니라 ORA-01555 뒤 데이터 문제다.

11.3 이 원천 세션에 `TIME_ZONE = '+00:00'` 을 못박은 것과 같은 이유이며, 여기까지 규정하지 않으면 그 고정이 Control 쪽에서 풀린다.

**`fence_ts` 는 예외가 아니라 표시 대상이다.** 저장은 위 규칙대로 `timestamptz`(UTC)지만 **값의 출처가 standby 시계**이므로 `fence_ts` 를 읽는 모든 곳은 그 사실을 전제해야 한다 — 12.2 가 이미 규정한다: `fence_ts`는 standby wall-clock이므로 primary 시계로 찍히는 `UPDATE_DT`의 하한으로 쓰지 않는다. **그 경고는 `fence_ts` 하나에 대한 것이고 나머지 컬럼은 Control 호스트 시계다.** 서로 다른 시계의 값을 빼는 곳은 위 표의 세 번째 항(undo deadline) 하나이며, 그것이 성립하는 근거는 두 시계 모두 UTC 절대 시각이라는 것뿐이다 — **naive 저장에서는 그 근거가 사라진다.**

**배포 요건.** Control 을 구성하는 모든 프로세스(Control API · 5.4 단일 scheduler · Run Pod · Outbox publisher)는 **DB 세션 `TimeZone` 을 `UTC` 로 고정**한다(22장 22번 config 표에 `control_session_timezone = UTC` 로 등록). PostgreSQL 서버 기본값에 의존하지 않는다 — 기본값은 배포마다 다르고 바뀌어도 알림이 없다.

**적용 범위.** 이 규약은 Control DB 컬럼에 대한 것이다. Iceberg 대상 컬럼 타입(NTZ vs timestamptz)은 별개이며 22장 12번이 확정한다.

#### 운영자 조회 표면 (v1.2.4 신설 소절 — D-01·A-19)

**결정(v1.2.4).** 운영자 UI 의 **모든 조회**는 Control DB read replica 를 읽는다. 16.2 가 "contract 단위 조회"에 대해 정한 것을 **목록·검색·필터까지 넓힌다.** 조회를 Control API 로 옮기지 않는다 — 다만 아래 두 경로는 예외이며 17장의 API 다.

**UI 는 6.1 테이블을 직접 읽지 않고 `etl_ui` 스키마의 read-only 뷰만 읽는다.**

```text
UI  ── 읽기 ──▶  etl_ui.*  (read replica 의 뷰)  ──▶  6.1 테이블
UI  ── 쓰기 ──▶  Control API (primary, 17장)
기준서 §5.1 판정 쿼리 ──▶ 6.1 테이블 직접 (16.2 그대로. 이 소절이 바꾸지 않는다)
```

이유 둘. ① 6.1 은 v1.2.1~v1.2.4 사이에 컬럼을 계속 늘렸다(`rebind_count`·`terminal_ingested_at`·`lease_grant.stage`·위 시각 규약) — UI 가 테이블에 직접 결합하면 규범 개정마다 UI 가 깨지고, **A 는 자기 개정의 영향 범위에 UI 를 세지 않는다.** ② 권한을 테이블 전체가 아니라 뷰 단위로 걸 수 있다.

**뷰 목록**(운영자 화면에서 역산한 최소 집합. 컬럼은 DDL 부록에서 확정한다)

| 뷰 | 담는 것 |
|---|---|
| `etl_ui.v_source_list` | SourceSystem + ACTIVE connection/credential revision + utilization ρ + 열린 hold 수 |
| `etl_ui.v_job_list` | Job + ACTIVE release + 16.2 파생 4항 + 최근 계약 상태 |
| `etl_ui.v_open_hold` | 열린 `maintenance_hold` + scope 전개(어느 Job 을 덮는가) |
| `etl_ui.v_awaiting_human` | `ADJUDICATION_PENDING` · `DQ_FAILED` · `RECONCILIATION_REQUIRED` · `EMPTY_FULL` |
| `etl_ui.v_contract_detail` | contract + 이력 3종 + `guard_result` + `attempt_timeline` |
| `etl_ui.v_evidence` | `commit_evidence_ledger` + watermark 이력 + lease 현황 |
| `etl_ui.v_release_usage` | release → 그 release 를 쓰는 Job (rollback 영향 범위) |

**`v_open_hold` 의 scope 전개가 뷰여야 하는 이유는 성능이 아니라 일관성이다.** Hold 겹침 의미(effective mode = `max(FORCE_STOP > DRAIN > HOLD_NEW)`, 14.1)가 조회하는 쪽마다 구현되면 화면끼리 같은 Job 에 다른 답을 낸다.

**replica 조회로 대신할 수 없는 것 둘 — 17장의 API 다.**

1. **되돌리기 어려운 명령 직전의 재확인** — `dq:accept`·`accept-empty`·`resolve`·`watermark:seed`·`db-identity:rotate`·`force` revoke·`abort`. 지연된 값으로 확인 화면을 채우면 **운영자가 이미 없는 상태를 보고 결정한다.** `GET /v1/contracts/{id}` · `GET /v1/jobs/{id}`(17장 신설).
2. **Job 생성·개정 경로** — `drafts → advisor-analyses → validate → publish` 는 순차 의존이므로 replica lag 안에서 앞 단계 결과를 읽으면 없는 draft 를 참조한다.

그리고 **명령의 결과는 그 명령의 응답으로 읽는다.** 쓰기 뒤 화면 갱신을 replica 재조회로 하지 않는다 — 그래서 17장이 `POST /v1/holds` 응답에 영향 범위를 싣는다.

**권한(A-19 — 제약 (9) 의 일반화).** v1.2.3 까지 `target_lease`·`target_table` 두 테이블에만 있던 규칙을 6.1 전체로 넓힌다.

> **6.1 의 모든 테이블에 대해 DML 권한은 Control API role 에만 부여한다.** 운영·분석 role 은 테이블 SELECT 만 갖고, **UI role 은 테이블이 아니라 `etl_ui.*` 뷰에 대해서만 SELECT 를 갖는다.**

raw session 이 lease row 를 직접 넣는 경로를 막던 것과 같은 이유이며(기준서 FI-51 변형), 여기에 **UI 가 6.1 스키마에 결합하는 경로**를 막는 것이 더해진다.

> **인가(누가 무엇을 할 수 있는가)는 이 소절이 정하지 않는다** — 22장 13번이다. 이 소절이 정하는 것은 **어느 주체가 어느 객체를 읽을 수 있는가**뿐이다.
...
- `ORA-01555`·`ORA-01466`(SCN 이후 DDL)·`ORA-08181`(**v1.2.4 정정** — '유효하지 않거나 너무 오래된 SCN'이 아니라 **공급된 SCN이 유효 SCN 범위 밖**이라는 뜻이다: *"The supplied SCN was beyond the bounds of a valid SCN"*. '너무 오래되어 image가 덮인' 경우는 `ORA-01555`이고 이 목록에 이미 있다. 08181의 대표 사례는 **다른 DB의 SCN**이며 그것은 이 절 앞 문단의 `db_identity` 대조가 막는다. `AS OF TIMESTAMP` 경로를 채택하면 *"no snapshot found based on specified time"*인 **`ORA-08180`**이 이 목록에 더해져야 하는데, 그 경로는 아직 채택하지 않았으므로 지금 넣지 않는다 — 쓰지 않는 경로의 오류 코드를 규범에 미리 박으면 채택한 것처럼 읽힌다. 8차 교차 리뷰 §4.1·M4-2)은 receipt `committed=false, exception_class`로 보고되는 `attempt-failure {reason: SPARK_FAILED}`이며 **같은 계약에서 RETRY를 금지**한다(v1.2.1 — v1.2의 ‘chunk 축소’는 삭제: 같은 `visible_scn` 재독은 무의미하고 chunk를 줄여도 덮인 undo는 돌아오지 않는다). Control은 이 `exception_class`를 `FENCE_EXPIRED`와 같은 처리로 둔다(10.2 복구 경로 (c): `ADJUDICATION_PENDING` 유지, `resubmit_blocked = true`, `retry_authorization` 무효, RETRY API 409 `FENCE_EXPIRED`) — 운영자 `abort` 후 다음 회차가 `[current_watermark, fence_new)`를 덮고 Full은 `RERUN_LATEST`. 접두 chunk의 CAS는 보존된다(13.2 `PARTIAL_COMMIT`)
...
**`AdvisorRecommendation` 형식(v1.2.4 신설 — A-8).** 위 세 줄이 저장을 요구하는 값들의 필드 이름을 확정한다. v1.2.3까지 "confidence 와 근거 metadata ID 제공"·"버전 저장"만 있고 이름이 하나도 없어, 저장할 수도 화면에 올릴 수도 없었다.

| 필드 | 뜻 |
|---|---|
| `target_field` | 이 추천이 채우려는 **JobSpec 키**. 17장 `violations[].field` 와 같은 어휘를 쓴다 — 검증 위반이 어느 추천에서 왔는지 잇는 키다 |
| `value` | 추천값 |
| `confidence` | `[0,1]`. **판본 간 비교 가능한 척도가 아니다** — `versions` 와 함께 읽는다 |
| `evidence_ids[]` | 근거 metadata ID(Oracle dictionary 항목·DataHub 항목) |
| `evidence_kind` | `DICTIONARY` \| `DATAHUB` \| `SIMILAR_JOB` \| **`SOURCE_COMMENT`** \| `COLUMN_NAME_PATTERN`. 뒤 둘은 신뢰 등급이 다르다 — `SOURCE_COMMENT` 는 아래 prompt injection 분리 대상이고, `COLUMN_NAME_PATTERN` 은 "컬럼명만으로 확정하지 않음" 규칙에 걸린다 |
| `versions` | `{model, prompt, rule, metadata}` — 위 "버전 저장"의 4항 |

**`AdvisorAnalysis` 의 결속(v1.2.4 — A-9).** `AdvisorAnalysis` 는 6.1 에서 **`Job` 하위**이며 `(job_id, draft_revision)` 으로 그 분석이 **어느 입력에 대한 것인지**를 기록한다. v1.2.3까지 이 객체는 최상위 형제였고 Job 과의 연결이 17장 경로 파라미터뿐이라, **draft 가 수정된 뒤 기존 추천이 낡았다는 것을 말할 방법이 없었다** — 낡은 추천이 새 draft 의 추천처럼 보인다. Wizard 가 앞 단계로 돌아가 입력을 바꾸면 `draft_revision` 이 올라가고, 그보다 낮은 분석은 **stale 로 표시**한다(자동 폐기하지 않는다 — 운영자가 무엇이 바뀌었는지 볼 수 있어야 한다).

**수락·거절의 기록(v1.2.4 — A-11).** 운영자가 추천을 적용·거절·무시한 사실을 `AdvisorAnalysis` 에 남기고 최종 JobSpec 값과 함께 audit 한다. **승인자는 요구하지 않는다** — 17장의 `dq:accept`·`accept-empty`·`resolve`·`watermark:seed`·`db-identity:rotate`·`force` revoke 일곱 경로가 `사유·승인자 필수`인 것은 그것들이 **안전 판정을 뒤집기** 때문인데, 추천 수락은 뒤집는 것이 없고 7.2 13번 validator 를 그대로 통과해야 한다. **이 비대칭은 누락이 아니라 의도이며 v1.2.4 가 그것을 명시한다.** 다만 기록 자체는 필수다 — 15.4 2단계의 "운영자 수정률 측정"이 그것을 입력으로 쓴다.

**무응답도 기록한다.** Wizard 를 그냥 지나간 추천은 `NOT_APPLIED` 이며 "미검토" 상태를 남기지 않는다 — 남기면 뒤에 "검토했다"로 읽힌다.
...
- Custom UI: Source, Job Wizard/CRUD(**Advisor 추천 확인은 그 안의 3단계다** — v1.2.4 정합, A-12: v1.2.3까지 이 목록이 `Advisor` 를 `Source`·`Job Wizard/CRUD` 와 나란히 적어 독립 화면을 시사했는데 7.2 는 Wizard 단계로만 둔다. 두 절이 어긋났고 7.2 를 권위로 삼는다. **독립 화면이 필요한 것은 추천 화면이 아니라 15.4 1~2단계의 Shadow 평가 지표**(안전하지 않은 추천률·운영자 수정률)이며 그 화면은 22장 13번 확정 뒤에 정한다), Template/Release, Hold, Backfill plan, **contract 상세**(상태 이력·Guard 결과·attempt timeline — 6.1의 `contract_state_history`·`attempt_state_history`·`lease_state_history`·`guard_result`·`attempt_timeline`. `ADJUDICATION_PENDING`·`RECONCILIATION_REQUIRED`·`DQ_FAILED`에서 운영자가 RETRY/abort/resolve/dq:accept를 고르는 근거 화면이며 Dagster UI의 run 이력을 복제하지 않는다; `DQ_FAILED` 계약은 ‘main 이미 노출’ 배지와 노출 `committed_snapshot_id`를 첫 줄에 표시한다 — 13.1, v1.2.1)
...
- contract·attempt 단위 감사·지표의 원천은 Control DB의 이력·계측 테이블(6.1)이다. Prometheus/Grafana로는 Job class·Source·Guard 사유 단위로 집계한 값만 내보내고(위 label 규칙 유지), **운영자 UI 의 모든 조회**(contract 단위 조회를 포함해 Source·Job·Hold 목록과 검색·필터까지)는 Custom UI가 Control DB(read replica)의 **`etl_ui` 뷰**를 읽는다(v1.2.4 — 6.1 「운영자 조회 표면」 소절이 권위이며 뷰 목록·권한·예외 2건을 규정한다. v1.2.3까지 이 문장은 "contract 단위 조회"만 덮었다). PoC 기준서 §5.1 판정 쿼리는 운영에서도 같은 테이블에 대해 실행한다
...
POST   /v1/jobs/drafts
GET    /v1/jobs/{id}                             # v1.2.4 신설 (A-17) — 200 {job_id, active_release_id, draft_revision, …}. 되돌리기 어려운 명령 직전의 재확인과 Wizard 개정 경로는 replica 를 읽지 않는다 (6.1 「운영자 조회 표면」)
POST   /v1/jobs/{id}/advisor-analyses            # v1.2.4 계약 확정 (A-9·A-10) — 요청 {draft_revision, targets[]?}; 200 {analysis_id, draft_revision, recommendations[]} | 200 {analysis_id: null, available: false, reason} (Advisor 불가 — 7.2 3번 "선택 단계, 장애 시 건너뜀"이므로 5xx 로 내지 않는다: 선택 단계의 장애를 오류로 만들면 client 가 재시도하게 되고 15.2 의 저부하 요건과 어긋난다) | 422 (draft 없음·targets 무효). `recommendations[]` 형식은 15.3
...
GET    /v1/contracts/{id}                        # v1.2.4 신설 (A-17) — 200 {contract_state, current_attempt, verdict?, window_kind, next_eligible_at, …}. 판정 직전 재확인 전용이며 목록 조회는 etl_ui 뷰다 (6.1)
POST   /v1/contracts/{id}/retry
POST   /v1/backfill-plans
POST   /v1/backfill-plans/{id}/start

POST   /v1/holds                                 # v1.2.4 응답 확정 (A-18) — 201 {hold_id, scope_kind, scope_key, mode, reason, impacted_job_count, effective_mode_changed_job_count}. 쓰기 뒤 화면 갱신은 이 응답만으로 이뤄져야 한다(replica 재조회는 지연 안에서 방금 한 일을 보여 주지 못한다, 6.1 「운영자 조회 표면」). 14.2 해제 쪽이 이미 T_catchup·impacted 를 다루므로 생성 쪽을 같은 모양으로 맞춘 것이다
...
10. ~~Dagster direct UI 실행 허용 여부~~ → 14.3에서 확정(Guard가 NORMAL 계약 생성, Critical 거부). 대신 **Dagster UI 노출 정책**: read-only 인스턴스 분리, write 인스턴스의 SSO proxy·mutation allowlist·감사. **v1.2.4 — 여기서 확정하는 proxy access log 의 주체 표기가 6.1 `AuditEvent.actor`(사번)와 같아야** 10.2 표의 `OPERATOR_CANCELLED` actor 보강이 Control 기록과 이어진다
11. Source별 용량 모델 계수(D 분포, ρ = Σ_i(D_i × w_i / period_i) / C_S — 19장 H-11과 같은 식, ρ 임계 0.7/1.0)와 publish Gate 운영
12. 전사 timestamp 타입(NTZ vs timestamptz)과 타입 매핑표 확정 — **v1.2.4 로 범위가 좁아졌다**: Control DB 시각 컬럼은 6.1 「시각 컬럼 규약」이 `timestamptz`(UTC)로 확정했으므로 이 항목은 **Iceberg 대상 컬럼 타입**만 남는다(7.3 `timestamp_type` 잠정 기본 `TIMESTAMP_NTZ`, 12.4 매핑표)
13. Control API 역할 모델·Run Pod 신원. 2인 승인/SoD는 정책 선택사항. **v1.2.4 — 세 가지를 나눠 둔다**: ① **인가**(누가 무엇을 할 수 있는가)가 이 항목이고 미확정이다 ② **인증**은 17장이 사내 SSO OIDC 로 이미 정했으며, 그 전 단계의 ID/PW 도 6.1 `auth_method` 에 같은 형식으로 기록한다 ③ **감사**(누가 했는가)는 6.1 `AuditEvent` 가 이미 요구하므로 인가 확정을 기다리지 않는다. 이 항목이 함께 확정할 것 하나 — **OIDC 의 어느 claim 을 `actor`(사번)에 싣는가**. `sub` 는 보통 불투명 식별자라 사번이 아니다
...
22. Job별 freshness SLO 수치와 운영 파라미터 기본값. v1.2는 PoC 기준서 §7의 시작값을 **잠정 기본값**으로 채택하고 기준서 결과(H-04·H-12·FI-02·FI-34·FI-35)로 확정한다. `freshness_slo`의 계산 주체·공식·dedup·분해 지표는 16.4에서 확정했고 수치만 잠정이다. 규칙으로 묶인 값은 규칙이 기본값이며 publish·기동 검증에서 위반을 거부한다:
    ...
    - **`control_session_timezone` = `UTC`**(v1.2.4 신설, A-13 — Control 을 구성하는 모든 프로세스(Control API·5.4 단일 scheduler·Run Pod·Outbox publisher)의 DB 세션 `TimeZone`. 잠정값이 아니라 고정값이다. PostgreSQL 서버 기본값에 의존하지 않는 이유는 그 값이 배포마다 다르고 바뀌어도 알림이 없기 때문이며, 6.1 「시각 컬럼 규약」의 `timestamptz` 저장과 짝을 이룬다)
